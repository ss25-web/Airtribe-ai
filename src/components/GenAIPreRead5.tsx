  ...
  return (
    <>
      ...
      <ChapterSection id="genai-m5-loops" num="01" accentRgb={ACCENT_RGB} first>
        {para(track === 'tech'
          ? "In Pre-Read 04, Aarav built the first automated exception classification workflow end-to-end — trigger, data transform, AI node, Slack notification, error handling, and a live test against real data. The workflow runs reliably on one exception at a time. This pre-read is about the gaps that show up the moment real volume arrives: 247 exceptions in a backlog, field names that don't match the prompt template, a confidence score that should pause for human review, and a user who wants the agent to remember what they said two messages ago."
          : "In Pre-Read 04, Rhea automated her Monday exception summary — she mapped her manual process, wired it into n8n, set up credentials, added error handling, and tested it against live data before going to her director. The workflow runs. This pre-read is about what breaks at scale: 80 rows that need looping, a field called 'acct_ref' where the prompt expects 'account_name', an approval step before any email goes out, and a conversation assistant that forgets everything between sessions."
        )}
        <SituationCard accent={ACCENT} accentRgb={ACCENT_RGB} label={track === 'tech' ? '◎ Aarav\u2019s Situation' : '◎ Rhea\u2019s Situation'}>
          {track === 'tech'
            ? "The M04 classifier handles one exception per trigger. This week's backlog: 247 claims. Running the workflow 247 times manually is not automation — it's clicking with extra steps. Aarav needs the workflow to iterate."
            : "Rhea's renewal workflow processes one row at a time. The Google Sheets export has 80 rows. She needs the workflow to loop over every row, transform each one, and aggregate the results before the report goes out."}
        </SituationCard>
      ...
      <ChapterSection id="genai-m5-transforms" num="02" accentRgb={ACCENT_RGB}>
        <SituationCard accent={ACCENT} accentRgb={ACCENT_RGB} label={track === 'tech' ? '◎ Aarav\u2019s Situation' : '◎ Rhea\u2019s Situation'}>
          {track === 'tech'
            ...
            : "Rhea's workflow pulls data from Google Sheets and a webhook. The sheet column is 'Policy Number'. The webhook field is 'policyNum'. The merge step finds zero matches — because it joins on field name, and those two strings are not equal."}
        </SituationCard>
